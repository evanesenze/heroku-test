...
const BaseBlocks = Object.entries(Base)
  ...
  .reduce(
    (acc, [key, value]) =>
      Object.assign(Object.assign({}, acc), { [String(key)]: value }),
    {}
  );
const exitHandler = [
  ...
  (ctx) => {
    ...
    return yandex_dialogs_sdk_1.Reply.text(
      "Хорошо, будет скучно - обращайтесь.",
      { end_session: true }
    );
  },
];
...
const wrongHandler = (ctx) => {
  ...
  if (!currentScene)
    return yandex_dialogs_sdk_1.Reply.text(
      "К сожалению я не поняла, что Вы хотели сказать, повторите пожалуйста."
    );
...
const sceneMessages = {
  ...
  SELECT_MENU_SCENE: [
    "Я могу найти стих по автору или по названию. Также можете взглянуть на рейтинг",
  ],
  ...
  if (compareText(text, ctx.message)) {
    ...
    if (
      ...
      if (currentBlock.isLast) {
        ...
        return yandex_dialogs_sdk_1.Reply.text(
          getCurrentText(
            Object.assign(Object.assign({}, learnData), { textType: "full" })
          )
        );
      }
      ...
      if (
        ...
        const nextLearnData = Object.assign(Object.assign({}, learnData), {
          currentBlock,
          textType: "full",
        });
        ...
        const nextLearnData = getNewLearnData(
          poem,
          "block",
    ...
    return yandex_dialogs_sdk_1.Reply.text(
      `Вы допустили ошибку. Повторите еще раз\n\n${text}`
    );
  }
});
...
atSelectMenu.command(/назван/i, (ctx) => {
  ...
  const message = String(
    (0, lodash_1.sample)(sceneMessages["SELECT_BY_NAME_SCENE"])
  );
  return yandex_dialogs_sdk_1.Reply.text(message);
});
atSelectMenu.command(/писател|автор|имя|имени/i, (ctx) => {
  ...
  const message = String(
    (0, lodash_1.sample)(sceneMessages["SELECT_BY_AUTHOR_SCENE"])
  );
  return yandex_dialogs_sdk_1.Reply.text(message);
});
atSelectMenu.command(/рейтинг/i, () =>
  yandex_dialogs_sdk_1.Reply.text("Рейтинг стихов можете посмотреть на сайте", {
    buttons: [
      yandex_dialogs_sdk_1.Markup.button({
...
atSelectMenu.any((ctx) => {
  var _a, _b, _c;
  const entities =
    (_a = ctx.nlu) === null || _a === void 0 ? void 0 : _a.entities;
  ...
  const items = Object.values(res).sort(
    (a, b) =>
      (0, js_levenshtein_1.default)(a.author + a.title, q) -
      (0, js_levenshtein_1.default)(b.author + b.title, q)
  );
  if (items.length) {
    const buttons = items.map(({ title, author }, i) =>
      yandex_dialogs_sdk_1.Markup.button(`${i + 1}). ${author} - ${title}`)
    );
    saveSelectListData(ctx.session, {
      items,
      offset: 0,
      query: q,
      key: "author",
    ...
    return yandex_dialogs_sdk_1.Reply.text(
      'Вот что я нашел:\nДля перемещения скажи "Далее/Назад"',
      { buttons }
    );
  } else if (names === null || names === void 0 ? void 0 : names.length) {
    const name = `${
      (_b = names[0].first_name) !== null && _b !== void 0 ? _b : ""
    } ${(_c = names[0].last_name) !== null && _c !== void 0 ? _c : ""}`.trim();
    const res = findPoemsBy("author", name);
    const items = Object.values(res).sort(
      (a, b) =>
        (0, js_levenshtein_1.default)(a.author, name) -
        (0, js_levenshtein_1.default)(b.author, name)
    );
    if (!items.length)
      return yandex_dialogs_sdk_1.Reply.text(
        `Не нашел автора "${ctx.message}"`
      );
    const buttons = items.map(({ title, author }, i) =>
      yandex_dialogs_sdk_1.Markup.button(`${i + 1}). ${author} - ${title}`)
    );
...
atSelectByName.any((ctx) => {
  ...
  const items = Object.values(res).sort(
    (a, b) =>
      (0, js_levenshtein_1.default)(a.title, q) -
      (0, js_levenshtein_1.default)(b.title, q)
  );
  if (!items.length)
...
const saveSelectListData = (session, newData) =>
  session.set("selectListData", newData); // !
...
atSelectList.command("Далее", (ctx) => {
  ...
  if (!selectListData) return yandex_dialogs_sdk_1.Reply.text("error");
  const { offset, key, query } = selectListData;
  const newOffset = offset + 5;
  const res = findPoemsBy(key, query, newOffset);
  console.log(res);
  const newItems = Object.values(res).sort(
    (a, b) =>
      (0, js_levenshtein_1.default)(a[key], query) -
      (0, js_levenshtein_1.default)(b[key], query)
  );
  const buttons = newItems.map(({ title, author }, i) =>
    yandex_dialogs_sdk_1.Markup.button(
      `${newOffset + i + 1}). ${author} - ${title}`
    )
  );
  const text = String((0, lodash_1.sample)(sceneMessages["SELECT_LIST_SCENE"]));
  saveSelectListData(
    ctx.session,
    Object.assign(Object.assign({}, selectListData), {
      items: newItems,
      offset: newOffset,
    })
  );
...
atSelectList.command("Назад", (ctx) => {
  ...
  if (offset === 0) {
    const buttons = items.map(({ title, author }, i) =>
      yandex_dialogs_sdk_1.Markup.button(
        `${offset + i + 1}). ${author} - ${title}`
      )
    );
    return yandex_dialogs_sdk_1.Reply.text(
      "Вы не можете сделать шаг назад - это первый лист",
      { buttons }
    );
  }
  const newOffset = offset - 5;
...
atSelectList.any((ctx) => {
  ...
  if (entities === null || entities === void 0 ? void 0 : entities.length) {
    ...
    if (numbers.length) {
      ...
      const currentNumber =
        (_b = numbers.find((item) =>
          itemNumbers.includes(Number(item.value))
        )) === null || _b === void 0
          ? void 0
          : _b.value;
      console.log(currentNumber);
      const selectedPoem = items.find(
        (_, i) => i + offset + 1 === currentNumber
      );
      if (selectedPoem) {
        ...
        return yandex_dialogs_sdk_1.Reply.text(
          `Ты выбрал ${selectedPoem.author} - ${selectedPoem.title}\n\n${text}`.substring(
            0,
            128
          )
        );
...
alice.command("", () => {
  return yandex_dialogs_sdk_1.Reply
    .text(`Добро пожаловать в “Навык изучениия стихов”.
${(0, lodash_1.sample)([
  "Здесь вы можете выучить стихотворение.",
  "Я помогу вам выучить стихотворение.",
])}
Вы уже знакомы с тем, что я умею?`);
});
alice.command(/да|знаком/i, () =>
  yandex_dialogs_sdk_1.Reply.text(`Итак, что будем учить сегодня?
...
alice.command(/новый|новое|другое|найти|поиск/i, (ctx) => {
  ...
  const message = String(
    (0, lodash_1.sample)(sceneMessages["SELECT_MENU_SCENE"])
  );
  return yandex_dialogs_sdk_1.Reply.text(message);
});
...
alice.command(/запомни|запиши|запись|записать|запомнить/i, () =>
  yandex_dialogs_sdk_1.Reply.text(
    "К сожалению, я не умею записывать ваш голос. Перейдите на сайт",
    {
      buttons: [
        yandex_dialogs_sdk_1.Markup.button({
          title: "Перейти на сайт",
...
alice.command(/расскажи|умеешь|не/i, () =>
  yandex_dialogs_sdk_1.Reply.text(`Что ж, пора рассказать Вам обо мне.
...
alice.listen(port);
console.log(1);
const comparingLines = (str1, str2) =>
  str1.replace(/[.,/#!$%^&*;:{}=\-_`~()…]/gi, "") ===
  str2.replace(/[.,/#!$%^&*;:{}=\-_`~()…]/gi, "");
const findByTag = (queryOriginal) => {
  ...
  return Object.entries(Base)
    ...
    .reduce(
      (acc, [key, item]) =>
        Object.assign(Object.assign({}, acc), { [key]: item }),
      {}
    );
};
const findPoemsByAll = (query, offset = 0) => {
  ...
  return BaseItems.filter(({ author, title }) => (author + title).match(regExp))
    ...
    .reduce(
      (acc, item) =>
        Object.assign(Object.assign({}, acc), { [String(item.id)]: item }),
      {}
    );
};
